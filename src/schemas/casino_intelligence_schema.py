...
# ============================================================================
# CATEGORY 9: MT CASINO TAXONOMIES (7 FIELDS)
# ============================================================================

class MTCasinoTaxonomies(BaseModel):
    """MT Casino taxonomy classifications for WordPress theme integration"""
    
    # MT Casino Categories (mt-listing-category2)
    categories: List[str] = Field(
        default_factory=list, 
        description="MT Casino categories: crypto-casino, live-casino, mobile-casino, new-casino, etc."
    )
    
    # MT Casino Software Providers (mt_software)
    softwares: List[str] = Field(
        default_factory=list,
        description="MT Casino software providers: netent, microgaming, playtech, pragmatic-play, evolution-gaming, etc."
    )
    
    # MT Casino Licenses
    licences: List[str] = Field(
        default_factory=list,
        description="MT Casino license taxonomies: mga, ukgc, curacao, gibraltar, kahnawake, etc."
    )
    
    # MT Casino Languages
    languages: List[str] = Field(
        default_factory=list,
        description="Supported languages: english, german, french, spanish, italian, etc."
    )
    
    # MT Casino Currencies
    currencies: List[str] = Field(
        default_factory=list,
        description="Supported currencies: usd, eur, gbp, btc, eth, etc."
    )
    
    # MT Casino Payment Methods
    payment_methods: List[str] = Field(
        default_factory=list,
        description="Payment method taxonomies: bitcoin, ethereum, visa, mastercard, paypal, skrill, etc."
    )
    
    # MT Casino Restricted Countries (mt_restricted_countries)
    restricted_countries: List[str] = Field(
        default_factory=list,
        description="Countries where casino is restricted: usa, uk, france, etc."
    )


class CoinFlipCasinoFields(BaseModel):
    """CoinFlip theme specific casino fields for WordPress integration"""
    
    # Small Description - Displayed on single casino page as subtitle
    small_description: Optional[str] = Field(
        None, 
        description="Short subtitle description displayed on single casino page"
    )
    
    # Casino Features - Displayed in About section
    casino_features: List[str] = Field(
        default_factory=list,
        description="List of casino features displayed in About section: Live Chat, Mobile App, VIP Program, etc."
    )
    
    # Pros/Cons - Featured on single casino page
    pros: List[str] = Field(
        default_factory=list,
        description="Casino advantages/pros for single page display"
    )
    
    cons: List[str] = Field(
        default_factory=list,
        description="Casino disadvantages/cons for single page display"
    )
    
    # Get Bonus Message
    bonus_message: Optional[str] = Field(
        None,
        description="Custom bonus call-to-action message"
    )
    
    # Casino Website URL
    casino_website_url: Optional[str] = Field(
        None,
        description="Official casino website URL for external links"
    )
...
class CasinoIntelligence(BaseModel):
    ...
    # ========================================================================
    # CATEGORY 9: MT CASINO TAXONOMIES (7 FIELDS) - FOR WORDPRESS INTEGRATION
    # ========================================================================
    mt_casino_taxonomies: MTCasinoTaxonomies = Field(
        default_factory=MTCasinoTaxonomies,
        description="Category 9: MT Casino WordPress taxonomy classifications (7 fields)"
    )
    
    # ========================================================================
    # CATEGORY 10: COINFLIP THEME FIELDS (6 FIELDS) - FOR COINFLIP THEME INTEGRATION
    # ========================================================================
    coinflip_fields: CoinFlipCasinoFields = Field(
        default_factory=CoinFlipCasinoFields,
        description="Category 10: CoinFlip theme specific casino fields (6 fields)"
    )
