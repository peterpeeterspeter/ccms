...
class WordPressRESTPublisher:
    ...
    async def publish_post(self, 
        ...
        try:
            # 🔧 CRITICAL FIX: Clean content from raw answer format first
            cleaned_content = self._clean_raw_answer_format(content)
            
            # 🔧 NEW: Process embedded images in content FIRST (before formatting)
            processed_content, embedded_media_count = await self._process_embedded_images(cleaned_content)
    ...
    async def publish_casino_content_with_coinflip_optimization(self, 
                                    title: str, 
                                    content: str, 
                                    structured_casino_data: Optional[Dict[str, Any]] = None,
                                    status: str = None,
                                    featured_image_url: str = None,
                                    enable_coinflip_embedding: bool = True) -> Dict[str, Any]:
        """🎰 ENHANCED: Publish casino content with CoinFlip theme field embedding"""
        
        # Extract CoinFlip theme fields from structured data
        coinflip_fields = {}
        if structured_casino_data:
            # Extract small description
            if structured_casino_data.get('coinflip_fields', {}).get('small_description'):
                coinflip_fields['small_description'] = structured_casino_data['coinflip_fields']['small_description']
            
            # Extract casino features
            if structured_casino_data.get('coinflip_fields', {}).get('casino_features'):
                coinflip_fields['casino_features'] = ', '.join(structured_casino_data['coinflip_fields']['casino_features'])
            
            # Extract pros and cons
            if structured_casino_data.get('coinflip_fields', {}).get('pros'):
                coinflip_fields['pros'] = ', '.join(structured_casino_data['coinflip_fields']['pros'])
            if structured_casino_data.get('coinflip_fields', {}).get('cons'):
                coinflip_fields['cons'] = ', '.join(structured_casino_data['coinflip_fields']['cons'])
            
            # Extract other fields
            for field in ['bonus_message', 'casino_website_url']:
                if structured_casino_data.get('coinflip_fields', {}).get(field):
                    coinflip_fields[field] = structured_casino_data['coinflip_fields'][field]
        
        # If CoinFlip embedding is enabled, wrap content with embedded fields
        if enable_coinflip_embedding and coinflip_fields:
            embedded_content = self._embed_coinflip_fields_in_content(content, coinflip_fields)
        else:
            embedded_content = content
        
        # Call the original method with embedded content
        return await self.publish_casino_content(title, embedded_content, structured_casino_data, status, featured_image_url)
    
    def _embed_coinflip_fields_in_content(self, content: str, coinflip_fields: Dict[str, str]) -> str:
        """Embed CoinFlip theme fields as hidden data for theme to extract, keep original content intact"""
        
        # 🔧 CRITICAL FIX: Clean content from raw answer format first
        cleaned_content = self._clean_raw_answer_format(content)
        
        # Create structured data section for CoinFlip theme (hidden for theme JS to read)
        newline = '\n'
        structured_data = f'<!-- CoinFlip Theme Structured Data -->{newline}<div class="coinflip-casino-data" style="display: none;">{newline}'
        
        for field_name, field_value in coinflip_fields.items():
            css_class = field_name.replace("_", "-")
            structured_data += f'<div class="{css_class}">{field_value}</div>{newline}'
        
        structured_data += f'</div>{newline}{newline}'
        
        # Add structured data and meta tags for CoinFlip theme, but keep original content
        final_content = f'''<div class="mt-casino-listing coinflip-theme-optimized">
{structured_data}
<!-- CoinFlip Theme Meta Tags -->
<meta name="casino-small-description" content="{coinflip_fields.get('small_description', '')}">
<meta name="casino-features" content="{coinflip_fields.get('casino_features', '')}">
<meta name="casino-pros" content="{coinflip_fields.get('pros', '')}">
<meta name="casino-cons" content="{coinflip_fields.get('cons', '')}">
<meta name="casino-bonus-message" content="{coinflip_fields.get('bonus_message', '')}">
<meta name="casino-website-url" content="{coinflip_fields.get('casino_website_url', '')}">
...
{cleaned_content}
    ...
    def _clean_raw_answer_format(self, content: str) -> str:
        """🔧 CRITICAL FIX: Clean content from raw answer format to proper HTML"""
        if not content:
            return ""
        
        # Handle cases where content starts with answer=" format
        if content.startswith('answer="'):
            # Extract content between answer=" and the closing quote
            import re
            match = re.search(r'answer="(.+?)"(?:\s+sources=|$)', content, re.DOTALL)
            if match:
                raw_markdown = match.group(1)
                # Decode escaped newlines and format to HTML
                cleaned_markdown = raw_markdown.replace('\\n\\n', '\n\n').replace('\\n', '\n')
                # Convert markdown to HTML
                try:
                    import markdown
                    html_content = markdown.markdown(cleaned_markdown, extensions=['tables', 'fenced_code'])
                    logger.info("✅ Successfully cleaned and converted raw answer format to HTML")
                    return html_content
                except ImportError:
                ...
                logger.warning("⚠️ Could not extract content from answer format")
                return content
        
        # Handle cases where content is already a string representation of answer data
        if '"answer="' in content or 'answer="' in content:
            # Try to extract from string representation
            import re
            # Look for the actual content after answer="
            ...
            if match:
                ...
                try:
                    import markdown
                    html_content = markdown.markdown(cleaned_markdown, extensions=['tables', 'fenced_code'])
                    logger.info("✅ Successfully extracted and converted content from string representation")
                    return html_content
                except ImportError:
                    logger.warning("⚠️ Markdown not available, returning cleaned text")
                    return cleaned_markdown
        ...
        # If content looks like it's already HTML, return as-is
    ...
    async def upload_media_from_url(
        ...
        """
        🌐 Upload media from URL to WordPress (for Browserbase screenshots)
        
        Args:
            media_url: URL of the media file to upload
            filename: Desired filename for WordPress media
            alt_text: Alt text for accessibility
            caption: Media caption
            
        Returns:
            Upload result with success status and media_id
        """
        try:
            import aiohttp
            ...
            # Create media metadata for WordPress
            screenshot_metadata = {
                'url': media_url,
                'filename': filename,
                'timestamp': datetime.now().isoformat(),
                'capture_type': 'browserbase_screenshot'
            }
            
            # Use existing upload method with the downloaded data
            media_id = await self.upload_screenshot_to_wordpress(
                screenshot_data=media_data,
                screenshot_metadata=screenshot_metadata,
                alt_text=alt_text or f"Screenshot: {filename}",
                caption=caption or f"Visual evidence captured via Browserbase managed Chrome"
            )
            ...
            if media_id:
                ...
                return {
                    'success': False,
                    'error': 'Upload to WordPress failed'
                }
...
class WordPressIntegration:
    ...
    async def publish_content_with_browserbase_screenshots(
        self,
        title: str,
        content: str,
        browserbase_screenshots: List[Dict[str, Any]] = None,
        embed_style: str = "inline",
        **kwargs
    ) -> Dict[str, Any]:
        """
        🌐 Publish content with Browserbase screenshot integration (lean replacement for Playwright)
        
        Args:
            title: Post title
            content: Post content
            browserbase_screenshots: List of screenshot metadata from Browserbase toolkit
        ...
        try:
            # Process Browserbase screenshots if provided
            if browserbase_screenshots:
                logger.info(f"🌐 Processing {len(browserbase_screenshots)} Browserbase screenshots for WordPress")
                
                for i, screenshot_data in enumerate(browserbase_screenshots):
                    try:
                        # Extract screenshot metadata
                        screenshot_url = screenshot_data.get('storage_url')
                        category = screenshot_data.get('category', 'screenshot')
                        
                        if not screenshot_url:
                            logger.warning(f"⚠️ Screenshot {i+1}: No storage URL found")
                            continue
                        
                        # Generate WordPress-friendly filename and metadata
                        filename = f"{title.lower().replace(' ', '_')}_{category}_{i+1}.png"
                        alt_text = f"{title} - {category.title()} Screenshot (Browserbase)"
                        caption = f"Visual evidence of {title} {category} captured via Browserbase managed Chrome"
                        
                        screenshot_metadata = {
                            'filename': filename,
                            'alt_text': alt_text,
                            'caption': caption,
                            'source_url': screenshot_data.get('url', ''),
                            'category': category,
                            'capture_method': 'browserbase_managed_chrome',
                            'timestamp': screenshot_data.get('timestamp')
                        }
                        
                        # Upload screenshot to WordPress media library
                        upload_result = await self.upload_media_from_url(
                            screenshot_url, 
                            filename,
                            alt_text=alt_text,
                            caption=caption
                        ...
                        if upload_result.get('success') and upload_result.get('media_id'):
                            screenshot_media_ids.append(upload_result['media_id'])
                            logger.info(f"✅ Browserbase screenshot {i+1} uploaded: Media ID {upload_result['media_id']}")
                        else:
                            logger.warning(f"❌ Failed to upload Browserbase screenshot {i+1}")
                            
                    except Exception as e:
                        logger.error(f"❌ Error processing Browserbase screenshot {i+1}: {e}")
                        continue
            
            # Embed screenshots in content
            if screenshot_media_ids:
                content = await self.embed_screenshots_in_content(
                    content, 
                    screenshot_media_ids, 
                    embed_style
                )
                logger.info(f"📝 Embedded {len(screenshot_media_ids)} Browserbase screenshots in content")
            
            # Publish the post
            publish_result = await self.publish_post(
                title=title,
                content=content,
                **kwargs
            )
            ...
            publish_result['browserbase_screenshots'] = {
                'count': len(screenshot_media_ids),
                'media_ids': screenshot_media_ids,
                'embed_style': embed_style,
                'processed': len(browserbase_screenshots) if browserbase_screenshots else 0
            }
            
            return publish_result
            
        except Exception as e:
            logger.error(f"❌ Browserbase screenshot publishing failed: {e}")
            return {
                'success': False,
                'error': str(e),
    ...
    async def publish_content_with_screenshots(
        ...
        **kwargs
        ...
        """
        🖼️ [DEPRECATED] Publish content with Playwright screenshot integration
        Use publish_content_with_browserbase_screenshots for lean managed Chrome solution
        """
        screenshot_media_ids = []
